...
## CHANGES
## added line 17 to import py_secp256k1
## changed line 221 and 225 in the sign() to call the ecdsa_sign() specifically from py_secp256k1 
...
class PrivateKey(EmbitKey):
    ...
    def sign(self, msg_hash, grind=True):
        sig = Signature(py_secp256k1.ecdsa_sign(msg_hash, self._secret))
        if grind:
            counter = 1
            while len(sig.serialize()) > 70:
                sig = Signature(py_secp256k1.ecdsa_sign(msg_hash, self._secret, None, counter.to_bytes(32, 'little')))
                counter += 1
