## CHANGES
## added  line 426 for calculating the pub key hash 
## chnages line 427 to swap the input argument from self.secret (private key) to the calculated pub key hash for the 'k' derivation function
...
class ECKey:
    ...
    def sign_ecdsa(self, msg, nonce_function=None, extra_data=None, low_s=True):
        ...
        # calculating and sending the hash of the public key for deriving the 'k' value
        pub_key_hash_int_for_nonce = int.from_bytes(hashlib.sha256(self.get_pubkey().get_bytes()).digest(), "big")
        k = nonce_function(pub_key_hash_int_for_nonce, z, extra_data=extra_data)
